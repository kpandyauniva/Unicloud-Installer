...
ZONE=$(python /root/fetch-metadata.py zone)
PROJECT=$(python /root/fetch-metadata.py project-id)
MACHINE_TYPE=$(python /root/fetch-metadata.py clusterMachineType)
NUM_WORKERS=$(python /root/fetch-metadata.py numberOfWorkers)
GLUSER_DISK_SIZE=$(python /root/fetch-metadata.py glusterDiskSize)
CLUSTER_MACHINE_IMAGE=$(python /root/fetch-metadata.py clusterMachineImage)
...
function prepare_master(){
                EXEC_CMD="scp -q /root/yaml/* fedora@${master_node}:~fedora"
                execute_retry 2
